...
function hmacSha256(key, msg) {
  ...
  const kp = new Uint8Array(B);
  kp.set(k);
  const ipad = new Uint8Array(B),
    opad = new Uint8Array(B);
  for (let i = 0; i < B; i++) {
    ipad[i] = kp[i] ^ 0x36;
    opad[i] = kp[i] ^ 0x5c;
  }
...
function imageDigest(rgba, onProgress) {
  ...
  for (let i = 0; i < n; i++)
    rev.set(store.subarray((n - 1 - i) * 32, (n - i) * 32), i * 32);
  return sha256(rev);
}
...
self.onmessage = (e) => {
  ...
  const clock = () =>
    typeof performance !== "undefined" && performance.now
      ...
      : Date.now();
  const t0 = clock();
  const digest = imageDigest(rgba, (frac) =>
    self.postMessage({ type: "progress", pct: Math.floor(frac * 100) }),
  );
  const answer = hmacSha256(seed, digest);
  self.postMessage({
    type: "done",
    answer: b64url(answer),
