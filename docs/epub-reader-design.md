...
- **EPUB cover fallback + backfill (DW.11):** manga-chapter EPUBs routinely declare NO OPF cover (the real Jujutsu Kaisen import — `img/01.jpg…`, no `<meta name="cover">`), so `epub_cover_bytes` falls back from `get_cover()` to the first image RESOURCE by sorted path — the same page-1 rule as CBZ. For a batch imported before this existed, `backfill_book_covers` (`POST /admin/media/import/backfill-covers` + a console button) re-extracts a cover for any Epub/Cbz media lacking an image variant: idempotent, spawned (hundreds of books), logged. So a coverless import self-heals with one click instead of a re-import.
...
**Two front doors + the console (DW.3/DW.4, `GET /admin/media/import`).** (1) **Filesystem** (`POST …/import/filesystem`, series + a server-side folder path) — the 271-volume path, no upload; the folder is CANONICALIZED (resolves `..`/symlinks) + `is_dir`-checked, then the ingest is **spawned** (detached like the coordinator backfills — a bad pass can't take the app down) and logs its tally to `/admin/logs`, so the log tail + the series page filling in ARE the progress view. (2) **Browser** (`POST …/import/upload`, multipart) — SYNCHRONOUS for SMALL batches (`.epub`/`.cbz` parts only, others drained), returning the per-file report inline; an honest note steers a full series to the filesystem path. Both front doors accept EPUB and CBZ (`is_book_filename`), the `list_books` scan and the parse (`strip_book_ext`) handle both.
