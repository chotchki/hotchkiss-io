...
- **`git push origin main` deploys to production** — no staging step. The post-receive hook builds and swaps the live app, with ~10–15s of downtime during `launchctl kickstart -k`. A failed build aborts before the swap (running app keeps serving), but a *successful build of broken runtime behavior* goes live immediately. `origin` is the mini and the only remote (the `github` mirror was dropped 2026-06).
