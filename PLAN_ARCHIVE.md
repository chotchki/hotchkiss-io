...
*Recovered from the exhausted 52-agent registration audit (session f3aa0642, 2026-07-17): a real prod user's passkey registration failed with ZERO user feedback AND zero server-log evidence. Root cause confirmed in live code — the client ceremony fails silently on both sides. This phase makes every auth failure visible to the user AND the operator. Full findings dump: `scratchpad/reg_findings_full.md` (36 verified).*

- [x] DM.0 - Phase exit: no auth failure is silent — a cancelled/failed/unsupported registration or login tells the USER why (a real message in the ceremony UI, not a dead passkey sheet) AND leaves an OPERATOR-visible log line; duplicate/invalid names get real messages; every admin form surfaces its non-2xx. Tests + CLAUDE.md; shipped.
- [x] DM.1 - Register ceremony's missing `.catch` (THE incident): the `webauthn-register` chain (htmx-webauthn.js:205-221) has no `.catch`, so every `navigator.credentials.create()` rejection — user dismisses/cancels the passkey sheet, taps outside it on iOS, 60s timeout, hybrid/QR flow fails, user-activation expired after the two awaited fetches — is a silent unhandled rejection (the `#error_message` div is written ONLY on a non-OK HTTP return, NEVER on a throw). Add a `.catch` that renders a real message, distinguishing user-cancel (`NotAllowedError`) from genuine failure; give the autofill chain's console-only `.catch` (185-187) the same user-visible treatment.
- [x] DM.2 - Honor `webauthn_conditional_support()`: both call sites (206, 167) DISCARD its boolean, so an unsupported browser/device (Safari <17.4, Firefox <119, Lockdown Mode, no platform authenticator) proceeds into an undefined `parseCreationOptionsFromJSON` throw (127) → into the same catch-less chain. Gate on the boolean + render an explicit "passkeys aren't available here" state instead of a silent throw.
- [x] DM.3 - Fix the line-53 ReferenceError (`response.status` → `auth_opt_response.status`) that turns a would-be-visible auth-options error into a console-only ReferenceError, and re-sweep htmx-webauthn.js with Biome for the same undefined-var / un-interpolated-`${…}` class (Biome caught this family once already).
- [x] DM.4 - No-JS / htmx-didn't-load fallback: Register's native form submit falls through to `/login/not-needed` (and the top-nav + editor create forms to `GET /login?username=`) → a silent 401 + HX-Redirect reload loop that looks like nothing happened. Render an explicit "JavaScript + passkeys required" state instead of a mystery reload.
- [x] DM.5 - Registration handlers emit ZERO tracing (login.rs:152-215 — `logout` even has a `debug!`, registration has nothing): add attempt/success/new-user lines at info and the "Registration not in progress" + "Passkey already registered" branches at warn, so a registration (success OR fail) always leaves a log line. This is why the incident left no server-side evidence even for failures that DID reach the box.
- [x] DM.6 - Duplicate `display_name` (it IS the PK, `0001_TableUsers.sql:6`) currently 500s with a bare trace UUID AFTER the passkey is already minted on the device (finish_registration:204) — stranding a real credential and re-500ing every retry with that name. Pre-check availability in `start_registration` and reject with "that name's taken" BEFORE minting; downcast the UNIQUE/PK violation in `UserDao::create` as the TOCTOU backstop (reuse the DK.1 `DuplicateSlug` sqlx-downcast pattern). Note the related fresh-UUID-per-start / `exclude_credentials:None` retry-poisoning (stranded passkeys later 500 "User not found" at login) — decide fix-vs-defer here. **DEFERRED → backlog** (the duplicate-NAME collision is fixed; the rarer finish-failure-after-mint path is its own item).
- [x] DM.7 - `display_name` rides the URL path raw (`start_register/<name>`) with no validation: `?` silently truncates the username, `/ # %` 404/400 with no tracing. `encodeURIComponent` client-side + validate/slugify server-side (or move it to the request body) with a real error on reject.
- [x] DM.8 - Site-wide `htmx:responseError` handler in base.html → render the response text as an inline/toast error, covering EVERY admin `hx-post`/`hx-delete` that currently no-ops silently on a defense-in-depth reject (the 409 last-admin guard, the CZ 400 not-assignable guard, any 404). Promotes the standing "Admin forms swallow non-2xx" tech-debt item — this is the general "tell the user what failed" fix the incident motivates.
- [x] DM.9 - Operator visibility for denials: raise middleware denials (authz 401/403, greylist 429) + the finish_register extractor/body rejections above TRACE (tuned so routine bot-403 noise doesn't drown the log) so the `/admin/logs` warn|error filter surfaces them; evaluate the `/admin/logs` newest-file-only 256 KiB tail horizon that structurally hid the ERROR lines while chris was hunting this. **DONE:** authz 403→`warn`/401→`info` in `require_admin_for_mutations`; `/admin/logs` walks back across `MAX_FILES` daily files (regression-tested). **Deliberately NOT done:** per-request greylist-toll logging (the `/admin/greylist` + analytics dashboards own it — per-request lines would flood the log) and the finish_register body-extractor rejection (stays at the `axum::rejection` TRACE — a real browser always sends valid JSON; a custom extractor is disproportionate).
- [x] DM.10 - Tests + docs: e2e that a cancelled/failed ceremony surfaces a visible error and a duplicate name shows "taken"; integration for the `htmx:responseError` toast + the new handler log lines; CLAUDE.md delta (the login/auth-feedback contract). Phase-exit sweep to PLAN_ARCHIVE.
- [x] DM.11 - Release step: bump the fab-gui pin `0.16.0` → `0.16.1` (`FAB_GUI_VERSION`/`FAB_GUI_TAG=web-v0.16.1` in build.rs) alongside the DM ship, `cargo clean -p hotchkiss-io` (marker short-circuit), verify the `/3d/editor` boots (`editor_boots_in_browser`). Ride the same beta→prod tag as the DM fixes.
...
- [x] DN.0 - Phase exit: a public "Open in the slicer" button on a model page opens fab-gui at `/3d/editor?model=/media/file/<scad-url_key>` and the model loads + slices; SCAD stored as `application/x-openscad`; the three.js mesh viewer is unaffected by the scad variant; tests + docs; shipped.
...
- [x] DP.0 - Phase exit: `GET /media/<ref>` negotiates (`?format=` > `Accept` > largest), `OPTIONS /media/<ref>` returns the role-aware hypermedia manifest, the slicer button loads via `?model=/media/<ref>?format=scad`; tests + docs; shipped.
- [x] DP.1 - `GET /media/<ref>` negotiation: `?format=<token>` (scad/stl/3mf/avif/mp4 → mime; 406 on no-match) > `Accept` (largest acceptable; `*/*` → largest; specific-unsatisfiable → 406) > largest. `Vary: Accept` + `Content-Location`; `min_role`-gated (denied ≡ 404); `Accept: application/json` → the item state.
- [x] DP.2 - `OPTIONS /media/<ref>` → the READ manifest `{ref, self, kind, title, min_role, variants:[{type,bytes,width?,height?,href}]}` (`build_manifest`, also `GET … Accept: application/json`); safe-method-public + `min_role`-gated; wired on the `/{media_ref}` method-router. The ROLE-AWARE `controls` block + per-variant `remove` move to DQ (they point at the write surface DQ builds — can't advertise dead links).
- [x] DP.3 - ONE shared, typed variant-SELECTOR (by-type / by-size over `ModelFormat` + `bytes`/`width`) used by the negotiation (DP.1), the manifest (DP.2), AND the embed (§8 — replacing its hand-rolled smallest-3mf / largest-mesh / srcset picks). No duplicate selection logic.
- [x] DP.4 - Slicer button → `?model=/media/<ref>?format=scad` (ref in the PATH = the SAVE target, derivable by dropping the query; format EXPLICIT — no implied state). Replaces the `?model=/media/file/<url_key>` form.
- [x] DP.5 - Tests (negotiation precedence + 406 + Vary; manifest shape + role-aware controls + gating; selector unit; slicer URL) + `media-design.md` [TARGET]→[SHIPPED] flips.
...
- [x] DT.0 - Phase exit: a register `create` can't collide with a pending autofill `get` or a double-tap; every client-side ceremony failure beacons `/login/ceremony_error` (logged with UA); a stuck user is told to reload; tests green; shipped.
...
## Phase DV - EPUB reader (foliate-js) + the manga library section

*The next Library media type after audiobooks: EPUB (manga + reflowable novels). Mirrors the audiobook shape (Phase DE) — a new `MediaKind::Epub`, a kind-specific embed, a client reader, a Family-gated `/library` section — but the reader is the **vendored foliate-js engine** (chris's call over a server-side page-extractor: it renders general EPUB incl. reflowable text, and offloads the format complexity — spine, RTL, fixed-layout, spreads — to a maintained engine, at the cost of downloading the whole `.epub` and a heavier JS dep). A manga SERIES is a flat subtree of `content_pages` (`library` → `manga` → `<series>` → `<volume>`, e.g. `<series>/1`, `/2`, `/3` — NO range grouping, decided out: pagination + `?q=` search on the series page handle a 271-volume series). The series page shows its volumes through a general **child-index markdown widget** — a fence the transformer expands into a paginated, searchable grid of child cards — so a series is just a normal content page with a picker in its markdown, no bespoke route. This phase makes a SINGLE volume work end to end (manual authoring); the 271-volume bulk path is DW. No site CSP today, and foliate needs no SharedArrayBuffer, so — unlike the fab-gui editor — NO COOP/COEP. Resume mirrors the audio player (`localStorage`, server-sync deferred like DF).*

- [x] DV.0 - Phase exit: an `.epub` uploads as an Epub media item; a Family-gated `/library/manga` section lists series → volume pages; a volume page renders the vendored foliate-js reader (RTL-aware, keyboard/swipe, `localStorage` resume), byte-served gated + range; tests (incl. a real-Chrome e2e) + docs; shipped.
- [x] DV.1 - Design doc `docs/epub-reader-design.md` (the source of truth, tagged like media/greylist/deadlinks): the foliate-js choice + the whole-`.epub`-download tradeoff (vs the rejected server-extract), the series/volume tree model + depth-agnostic serving, the gate (Family, per-volume byte URL), resume (`localStorage` `epub-loc:<ref>`, server-sync deferred), RTL/spread handling, and the honest limits (big-file first-load, headless-e2e scope).
- [x] DV.2 - `MediaKind::Epub`: `probe.rs` types `.epub` by extension (sibling to `.stl`/`.3mf`, before ffprobe) → mime `application/epub+zip`; the `MediaKind` enum + `as_str`/`from_str` gain `"epub"` (NO migration — a new string value). Verify the byte route serves it INLINE (the active-content neutralizer force-downloads svg/html/js — a zip mime is safe, but PIN it) with range + gate + `nosniff`. Unit test: `.epub` → Epub + mime.
- [x] DV.3 - Vendor foliate-js (pinned version, MIT) as ES modules under `assets/vendor/foliate/`, served by rust-embed (like threejs/katex, no build step); its zip reader via the already-vendored `fflate` or foliate's bundled zip. Document the pin + regen recipe.
- [x] DV.4 - `render_embed_html` Epub branch → the reader shell: a `<foliate-view>` mount + a boot splash that lifts on foliate's `ready` (like the editor), a download-`.epub` fallback for no-JS; loads `epub-reader.js`. Emitted with the `<span>`/flex wrappers the STL embed uses (a bare `![]()` sits inside a `<p>`).
- [x] DV.5 - `assets/scripts/epub-reader.js`: fetch `/media/file/<url_key>` as a Blob → open with foliate-js → mount the view; RTL auto from the OPF `page-progression-direction`; keyboard (←/→), tap-zones, swipe, spread toggle, fullscreen; `localStorage` resume keyed `epub-loc:<ref>` (foliate CFI, applied on ready — mirror `audio-player.js`). Vanilla, `defer`'d, degrades to the download link.
- [x] DV.6 - A **FLEXIBLE child-index markdown widget** — THE listing mechanism (chris: "make it flexible + restructure audiobooks under it"). A transformer fence ` ```children ` with params (`order=newest|manual`) emits a STABLE per-order sentinel (so `transform()` stays pure + content-hash-cached); the `get_page` handler fills it PER-REQUEST with the page's children via `listing.rs::paginate` — a RICH card (cover + title + excerpt + Scheduled/visibility badges, covering audiobooks AND manga), an admin **"+ new child"** form (authoring stays inline), and the shared `listing_search`/`listing_pager` partials. Reusable on ANY parent→children page; missing/empty → empty state, never a 500.
- [x] DV.7 - **Generic library-section route + audiobooks migration + manga section.** ONE `show_library_section` in `library.rs` (gate-check → render the section's `/pages/library/<section>` content page, which carries the widget) REPLACES the bespoke `show_audiobooks` + `audiobooks.html` (RETIRED). Extract the content-page render shared by `get_page_path` + the section route — they differ ONLY in the gate response (get_page's cat-404 oracle vs the section's state-aware sign-in copy). The `audiobooks` content page gets a ` ```children order=newest ` fence; `manga` is a new authored child with ` ```children order=manual ` on each series page. All Family-gated, inherit-on-create. The DE tests + `family_library_gate_login_next_and_entry` e2e stay green.
- [x] DV.8 - Tests: probe (.epub→Epub), embed (renders the reader + the byte URL), byte route (`application/epub+zip` inline, range, Family gate — mirror the audiobook gating test), the child-index widget (a page with children → the picker grid; empty → empty state), `/library/manga` (anon gate vs Family list). A tiny valid `.epub` fixture. PLUS a browser e2e (real Chrome as Family, serialized on `E2E_LOCK`, DR.3-style): a volume page → foliate `<foliate-view>` mounts + the first page paints + the splash lifts, no fatal console error (scoped honestly for headless foliate).
- [x] DV.9 - Docs: `media-design.md` kinds § (the Epub kind + reader embed) + CLAUDE.md (media kinds + the child-index widget + the `/library/manga` series/volume tree), pointing at `docs/epub-reader-design.md`.
- [x] DV.10 - Auto EPUB cover: at ingest, parse the OPF (EPUB3 `properties="cover-image"` / EPUB2 `<meta name="cover">`), extract the cover image from the zip, store it as an image variant of the item (a small `zip` + minimal XML read, server-side — separate from the client foliate reader) so `cover_url_for`/`cover_hero_for` auto-populate the card + hero, like the audiobook `attached_pic` poster. A cover-less/oddly-structured EPUB degrades to the 📖 placeholder, never fails the upload.
- [x] DV.11 - Content-derived card cover (dogfood polish): when a child-index card's page has NO explicit `page_cover_media_id`, derive the cover from the FIRST `![](/media/<ref>)` embed in that page's content → that media item's thumbnail (audio poster / EPUB OPF cover / image), via the shared `media_select` pick. So a book/volume page auto-covers the listing card with ZERO manual cover-setting (and DW's bulk uploader needn't set covers). Explicit page cover still wins.
...
- [x] DW.11 - EPUB first-image cover fallback (`epub_cover_bytes`: no OPF cover → first image resource by sorted path, the page-1 rule) + a `backfill_book_covers` re-extractor (`POST /admin/media/import/backfill-covers` + console button, spawned) for a coverless pre-fallback import; plus listing-search de-pollution (skip the markdown body for an all-hex query so a `/media/<ref>` UUID can't match a numeric search — the "80 → 35 results" bug). Tests + docs.
- [x] DW.12 - Series-card cover roll-up: a container card (a manga series — only a ` ```children ` fence, no cover/embed) borrows its first volume's cover via `child_rollup_cover` (page_order ascending → volume 1; single level, capped scan), wired into `render_children_grid` after the explicit-cover + own-embed checks. So `/library/manga` series tiles aren't blank after a volume-only import. Test: `series_card_rolls_up_its_first_volume_cover`.
- [x] DW.13 - Deeper roll-up: `child_rollup_cover` descends up to `ROLLUP_MAX_DEPTH` (3) levels (depth-first, first-child, page_order asc; `Box::pin` for the async recursion) so a `series → season → episode` tree covers the SERIES card from the first episode two levels down — the single-level DW.12 left it blank once a season layer was inserted. Test: `series_card_rolls_up_a_cover_two_levels_deep`.

## Phase DX - Listing/navigation ergonomics (content-loading dogfood)

*Two friction points chris hit while loading real content into deep trees + long listings. Both no-JS, both surgical.*

- [x] DX.1 - Clickable breadcrumbs: the ancestor trail on a deep page was plain text — now `pages::breadcrumbs_from_path(&pages_path)` precomputes a `Vec<Crumb>` (title + cumulative `/pages/…` href) on `GetPageTemplate` (the dead `pages_path` field dropped), drops the top section (nav carries it) + the leaf (you're on it), rendered as `<a>` links in `get_page.html`. Test: `breadcrumbs_are_clickable_ancestor_links`.
- [x] DX.2 - Editable jump-to-page pager: "Page N of M" became a native GET `<form>` with a bounded number input (min 1 / max total, defaults to current, hidden `q` to keep search sticky) submitting `base_path?q=…&page=N` — type a page + Enter instead of clicking Next 25× on a 26-page series; out-of-range is clamped by `paginate`, never a 500. Shared across blog/projects/child-index (all full-nav). Updated the 4 old "Page N of M" string assertions; new test `pager_is_an_editable_jump_to_page_form`.
...
- [x] DY.0 - Phase exit: a nested volume page shows prev/next sibling buttons (page_order = volume order); finishing a volume's audio with the screen ON navigates to the next sibling + auto-plays (best-effort — one tap if the browser blocks a gesture-less play); tests + docs + shipped.
...
- [x] DZ.0 - Phase exit: an audiobooks section renders square cover cards (no crop), manga stays 3:4, both `/library/<section>` and `/pages/library/<section>` agree, the `/library` index renders section cards via the widget; tests + docs + shipped.
...
Posting a photo from the phone was a papercut chain (navigate → ?edit → a bare file input → the ref top-inserted at caret 0). Now `/admin/capture` is the phone lane: camera-first (`capture="environment"`), ORIGINALS through the canonical `POST /media` (no client downscale — chris's explicit call; visitors get the auto-minted AVIF `srcset`), destination select defaulting to **library-only** (capture now, author later; caption = media title) with new-draft (CU sentinel, photo = cover) + append-to-recent one radio away, and capture.js auto-switching to append-on-that-draft so a session accretes into ONE post. The nav entry is a real Admin-gated special page (migration `0031` — reorderable, undeletable; the fat-finger camera + "+ New page" pills died). Dogfooding drove the deep fixes: **HEIC normalized at ingest** (probe types duration-less hevc as `image/heic`; the resize falls back to a one-frame ffmpeg decode and mints a capped-1920 full AVIF rung; `is_web_displayable_image` keeps heic out of every ladder/fallback) and **orientation baked into the rungs** (EXIF read off the decoder pre-consume; ffmpeg's default autorotate covers HEIC — `-autorotate` can't be passed a value, found the hard way). Editor papercuts fixed (camera toolbar button, append-at-end for a never-focused textarea, `p-1.5` targets), the admin switcher became ONE partial with a mobile `<details>` dropdown, and the beta PWA pin got host-aware inverted identity icons. Decided OUT: Web Share Target (no iOS PWA support), client-side downscale. Seeds planted: rotate+crop as re-runnable derivation params (Ideas), the 3d-sitemap gap (Backlog).

- [x] EB.0 - Phase exit: from the PWA on a real iPhone — snap → original uploaded → draft created (or appended) — fast enough to use mid-print; editor papercuts fixed; tests + docs; beta dogfood then prod tag
...
- [x] EB.4 - Real-iPhone verification on beta: snap from the PWA (HEIC path now normalized at ingest by EB.9 — confirm the embed renders off-Safari and the AVIF rungs minted), cellular-speed sanity with originals, capture-page one-handed usability
