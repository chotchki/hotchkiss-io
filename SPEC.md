...
- **Same-site links go relative on save.** `web/markdown/links.rs::rewrite_site_links` runs in `put_page_path`: absolute links + image `src`s pointing at the site's own host (`Settings.domain` + its `www.` variant, any scheme/port, via `AppState.domain`) are rewritten to root-relative, preserving query + fragment. **Why on save, not render:** the stored markdown becomes the canonical portable form — it works on prod, beta, and any future host, and the Atom feed inherits it. It edits only the matched URL substrings (longest-first, so a bare-domain match can't corrupt a longer path URL), leaving the author's formatting otherwise byte-for-byte intact — not a full AST reflow.
