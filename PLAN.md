...
- **Registration retry-poisoning: fresh user UUID per start + `exclude_credentials:None`** (Phase DM.6 deferral; findings 29/31 from the f3aa0642 audit). `start_registration` mints a NEW `Uuid::new_v4()` every attempt and passes `None` for excluded credentials, so a device whose ceremony minted a passkey but whose `finish` then failed (or was retried) strands that credential — a later discoverable login finds the passkey but no matching user row → `finish_authentication`'s "User not found" 500. DM.6 fixed the duplicate-NAME collision (the common path); this is the rarer finish-failure-after-mint path. Fix: pass the in-progress user's credentials as `exclude_credentials`, and/or key the pending registration so a retry reuses the same UUID. Low frequency (needs a finish failure), but a genuine dead-end for the affected device.
