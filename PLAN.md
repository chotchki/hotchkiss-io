...
- [ ] CB.0 - Phase exit: /admin/analytics usable on a 390px phone (tables + widgets visible, no page-wide horizontal scroll); /feed.xml carries blog posts AND project pages newest-first; /sitemap.xml + a Sitemap-directive robots.txt live with per-page meta description/canonical/OpenGraph; beta de-indexed; verified on beta then prod
...
- **Fix 3 stale browser e2e tests (pre-existing failures): top_nav_no_overflow_on_mobile (selector predates the hamburger nav), admin_new_post_form_and_slugify_on_blog (the +New post form moved to the admin bar), anonymous_forbidden_from_admin_dashboard (403 is now the styled "How about NO!" page, not plain text). All fail on clean main — update selectors/assertions to the current UI or retire them.** — added 2026-06-28.
